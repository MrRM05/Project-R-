...
extern void titleControler(OSContPad *pad,ushort i, ushort newbutton);// 0x800B1C40
extern void Na_SE_Fadeout(uchar); // 0x800CA388
extern ushort pause_flag; // 0x800DC5FC 
extern char *circuit_name[20];// 0x800EF8C9
extern char *circuit_name_j[20];// 0x800E7574
extern ushort map_number_chart[5][4];// 0x800F2BB4 //we called it g_CupArray
extern char *class_name[4];//  0x800E76CC
extern uint race_level;//  0x800DC548
extern POSXY  gp_pause_message_positions[8];// 0x800E85C0
extern char *ta_result_select_j[7];// 0x800E775C
extern void SetCursorMessageColor(int now_position,int num,int default_color);// 0x800921C0
extern short maximum_x;// 0x8015F6E8 
extern short minimum_x;// 0x8015F6EA
extern short maximum_y;// 0x8015F6EC
extern short minimum_y;// 0x8015F6EE
extern short maximum_z;// 0x8015F6F0
extern short minimum_z;// 0x8015F6F2
///////////////////////////////////////////////////////////////////////////////////////////////////
//                                                                                               //
//MARK: -                                   ITEMBOX                                              //
//                                                                                               //
///////////////////////////////////////////////////////////////////////////////////////////////////
extern float screen_view_angle[4];// 0x80150130
extern float check_display_range(FVector base ,FVector mark ,ushort camera_direction,float radius,float angle2,float limmit_distance);// 0x802B80D0
extern void create_modeling_matrix(AffineMtx mtx, FVector position, SVector angle);// 0x802B5F74
extern int set_matrix(AffineMtx affine,int mode);// 0x802B4FF8
extern void scaling_matrix(AffineMtx mtx,float scale);// 0x802B5F00
extern void translate_matrix(AffineMtx matrix,AffineMtx matrix2, FVector vect);// 0x802B5450
extern void move_itembox(Object *obj);// 0x802A1600
extern void move_fake_itembox(Object *obj);// 0x802A10F0
extern void move_special_itembox(Object *obj);// 0x802A156C
extern void out_of_screen(Camera *camera,Object *obj);// 0x80297230
extern void on_screen(Camera *camera,Object *obj);// 0x802972B8
extern void set_object_buffer (Object *obj,FVector pos,SVector angle,FVector velo,short category);//0x80296D10

extern uchar kwgetitem_sub(short rank, short pflag);// 0x8007ADA8

extern void display_fake_itembox(Camera *camera,Object *obj);// 0x802A171C
extern void display_itembox(Camera *camera,Object *obj);// 0x802A1EA0
...
extern ushort *lap_list[3];// 0x800E4570
extern void kwsprite32x16(int cx,int cy,ushort *addr);// 0x8004CB90
extern void kwsprite_xlu(int cx,int cy,uint alpha,ushort *addr,uint sizex,uint sizey,uint cutx,uint cuty);// 0x8004C9D8
extern void kwprintlaptime_xlu(int tx,int ty,uint alpha,int timer);//0x8004F950
extern int kwgoalcnt;// 0x8018D320
extern char kwbesttotalsw;// 0x801657E5
extern uint kwbestlap[3];// 0x80165658
extern char kwbestlapsw;// 0x801657E3
extern void kwprintlaptime(int tx,int ty,int timer);//
extern void kwprintlaptime_flash(int tx,int ty,int timer);//
extern ushort replay_flag;// 0x8015F890
...
extern void SetWord2_a(int x,int y,char *printText,int interval,float mulx,float muly); //0x80093324
extern void SetWord2_ap(int x,int y,char *printText,int interval,float mulx,float muly); //0x80093358
extern void SetWord3(int x,int y,char *printText,int interval,float mulx,float muly,int type); //0x8009338C
extern void SetWord3_a(int x,int y,char *printText,int interval,float mulx,float muly); //0x800936B8
extern void SetWord3_ac(int x,int y,char *printText,int interval,float mulx,float muly); //0x800936EC
extern void SetWord3_ap(int x,int y,char *printText,int interval,float mulx,float muly); //0x80093720
extern void SetWord3_acp(int x,int y,char *printText,int interval,float mulx,float muly); //0x80093754
extern void SetWord4(int x,int y,char *printText,int interval,float mulx,float muly,int type); //0x80093788
extern void SetWord4_a(int x,int y,char *printText,int interval,float mulx,float muly); //0x800939C8
extern void SetWord4_ap(int x,int y,char *printText,int interval,float mulx,float muly); //0x800939FC
...
extern Gfx* FillRect1ColorF(Gfx *buf, int x1, int y1, int x2, int y2, int r, int g, int b, int a);
...
extern Gfx* GraphPtr;//0x80150298
extern unsigned long* GraphPtrHamp;
extern uint GraphPtrOG;
...
extern long g_GameLapTable[8];    // 0x80164390
...
extern struct AnmObject g_hudAnimStruct[];//0x80165C18 
extern struct Hud g_hudStruct[4]; //0x8018CA70 | 0x8018CAF4 | 0x8018CB78 | 0x8018CBFC               

extern uchar UnknFlagShroomsTT;// 0x80165888 
extern uint itemboxtbl_pal[16];// 0x800E44F0
extern uint itemboxtbl_idx[16];// 0x800E4530
extern uint itemboxallocptr[4];// 0x80183E88
extern void kwanm_roulette_init(int ibox_ptr, int num);// 0x8007B254
extern int kwadd_sval(short *var,short max,short add); //0x80040FD8
extern void kwanm_next(int num);// 0x80072488        
extern int kwanm_forward(int ibx_ptr, int, int, int, int, int);// 0x80072
extern short kwanm_randomitem(int num,int player);// 0x8007AFB0
extern void kwanm_swap(int,short,int,int,int);// 0x80072D3C      
extern void kwanm_clrevflag(int num,uint val);// 0x800722CC
extern void kwchart_off(int);// 0x80073600
extern int kwanm_wait(int, int);// 0x8007278C                                 
extern void kwanm_finish(int num);// 0x80072428
extern int kwanm_jump(int, int);// 0x800726CC                            
extern int kwanm_bitcheckevflag(int num,uint val);// 0x80072320
extern int kwanm_bitoffcheckevflag(int num,uint val);// 0x80072354
extern int kwsteal_item(int, int);// 0x8007B040                             
extern void kwanm_setoutput(int, short);// 0x800729D0          
extern void kwanm_start(int num, int type);// 0x800723A4
extern int kwsub_sval(short *var,short min,short sub); // 0x8004106C
extern void kwanm_roulette(int num);//0x8007B34C
extern void kwanm_useditem(int num);//0x8007AC9C
extern void kwsprite40x32_ci8(int cx,int cy,ushort *paladdr,uchar *idxaddr);// 0x8004E4CC
extern void kwtexture2D_ci8_bl(int x,int y,unsigned short ang ,float scale,ushort *paladdr,uchar *idxaddr,Vtx *vtxaddr,int sizex,int sizey,int cutx,int cuty);// 0x80047910
extern void kwcheck_opening_start(void);// 0x8005A99C
extern uint kwscreen_enable;// 0x8018D188
extern void execute_item(Player *car);// 0x802B2FA0
...
extern ushort kartrank[8];// 0x80165270
extern uchar title_mode;// 0x8018EDEF
extern uchar d_points[8];// 0x8018D9C8 
extern uchar *additional_point;// 0x8018D9BC 
extern char d_points_rank[8];// 0x8018D9D0
extern uchar players;// 0x8018EDF3
extern int frame_counter;// 0x800DC54C
extern short name_to_num[8];// 0x80164478
extern void SortDPRanking(char *b_ptr);// 0x800A3A10
extern char k_skip_flg;// 0x8018D9D8
extern int oga_demo_skip_ok_flag;// 0x800DDB24
extern void SkipReset(void);// 0x800921B4

extern char *cup_name[5];// 0x800E7500
extern char popup_status[4][3];//  0x800E86AD
extern char play_mode[5];//  0x800E86AB
extern int cup_level;// 0x800DC540
extern short retry_count;// 0x8016556E
extern uint ogawa_rank[10];// 0x801643B8

extern void kawano_itoa(int num,char *ptr);// 0x800A7894

extern void ResultSub_time(int x,int y,int num,int num2);// 0x800A32B4
extern void ResultSub_point(ObjBlock *ptr,int x,int y,int num,int num2,char *pts_buf);// 0x800A3ADC
extern char *drivers_name_j[8];// 0x800E76A8
extern float goalintime[8];// 0x8015F898
extern ushort rank_to_num[10];// 0x80164360
extern void DispOB_result_time(ObjBlock *ptr);// 0x800A2EB8
...
extern void DispOBJBlock(ObjBlock *ptr);//0x8009F5E0
extern void gselControler(DecompController *controller, ushort arg1);
extern uchar gsel_mode;// 0x8018EDED
extern Gfx* FillRect1ColorFog(Gfx *buf, int x1, int y1, int x2, int y2, int r, int g, int b, int a); //0x80098DF8
extern uint graphPtrOG;// 0x80150298 //u32

extern uchar psel_mode;// 0x8018EDEE
extern uchar msel_mode;// 0x8018EDEC

extern void DOB_msel_anime_mapman(ObjBlock *ptr); //0x800AB314


extern void DOB_msel_anime_map2(ObjBlock *ptr);// 0x800AB290
extern void DOB_msel_name(ObjBlock *ptr);// 0x800AB260

extern void mselControler(OSContPad *pad,ushort i, ushort newbutton);// 0x800B3B58

extern uchar circuit_select;//0x8018EE0B
extern uchar cup_select;// 0x8018EE09
extern uint cup_number;// 0x800DC544 

extern void SetFadeOut_b(void); //  0x8009E208

extern float grid_x[8];// 0x80165210
extern float grid_z[8];// 0x80165230
extern void imageinit(void);// 0x8005D290
extern void initial_camera(float px,float py,float pz,uint roty ,uint mode,uint index);// 0x8001C4D0
extern uint sequence_mode;// 0x800DC50C
extern uint new_sequence_mode;// 0x800DC524
extern uint next_sequence_mode;// 0x80150124
extern ushort demo_flag;// 0x800DC51C
extern int race_screen_mode;// 0x800DC530
extern uint screen_mode; // 0x800DC52C 
extern void result_pos(void);// 0x8003CD78
extern void reset_pos(void);// 0x8003C0F0
extern Camera *camera1;// 0x800DDB40
extern Camera *camera2;// 0x800DDB44
extern Camera *camera3;// 0x800DDB48
extern Camera *camera4;// 0x800DDB4C
extern void init_draw(Player *car,Camera *camera,char kno,char place);// 0x8003CD98
